    ...
    <body>
        <div class="container">
            ...
            if ($validId != null)
            {
                ...
                if (is_int($validId))
                {
                    $objet = ObjetDAL::findById($validId);
                    if (is_null($objet))
                    {
                        echo "[DEBUG]Aucun objet trouvé avec cet ID...</br>";
                    }
                    else
                    {
                        ?>
                        <div class="row">
                            <div class="col-lg-12">
                                <form action=<?php $_SERVER['DOCUMENT_ROOT'] ?>"/controller/page/mod_objet.php" method="POST">
                                    <legend>Formulaire de modification d'un Objet</legend>

                                    <!-- Label de l'Objet-->
                                    <div class="col-lg-2">
                                        <label for="label" class="control-label">Label : </label>
                                        <input type="text" name="label" class="form-control" id="label" value="<?php echo $objet->getLabel(); ?>">
                                    </div>

                                    <!-- Description de l'Objet-->
                                    <div class="col-lg-4">
                                        <label for="description" class="control-label">Description : </label>
                                        <input type="text" name="description" class="form-control" id="description" value="<?php echo $objet->getDescription(); ?>">
                                    </div>

                                    <!-- Bouton de Validation-->
                                    <div class="col-lg-3">
                                        </br>
                                        <input type="submit" value="Modifier cet Objet" class="btn btn-success btn-block"/>
                                    </div>

                                    <!-- ID de l'Objet Caché et modification désactiver-->
                                    <div class="col-lg-1" hidden>
                                        <label for="id" class="control-label">Id : </label>
                                        <input type="text" name="id" class="form-control" id="id" value="<?php echo $objet->getId(); ?>">
                                    </div>
                                </form>
